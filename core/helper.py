...
from net.maml_mobilenet import MobileNetV1_Classifier
...
from net.maml_LSTM import LSTM_Classifier
from net.maml_1dcnn import Classifier_1DCNN
from net.maml_CLDNN import Classifier_CLDNN
from net.maml_complexcnn import Classifier_ComplexCNN
...
from core.dataset import OmniglotDataset, Jamming_img_Dataset, Jamming_seq_Dataset, Jamming_fusion_Dataset
...
def get_model(args, dev):
    ...
    # model = MobileNetV1_Classif-ier(1,  args.n_way).cuda()
    # model = Concat_Classifier(1, args.n_way).cuda()

    ## I/Q域   输入通道为2  I/Q两个通道
    # model = LSTM_Classifier(2, args.n_way).cuda()
    # model = Classifier_1DCNN(2, args.n_way).cuda()
    # model = Classifier_ComplexCNN(1, args.n_way).cuda()   ## ps.复数神经网络输入通道in_ch=1,只看虚部或实部通道数
    # model = Classifier_CLDNN(2, args.n_way).cuda()
...
def get_dataset(args):
    ...
    train_dataset = Jamming_img_Dataset(args.train_data_dir, args.task_num,
                                    n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
    val_dataset = Jamming_img_Dataset(args.val_data_dir, args.val_task_num,
                                  n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
...
# def get_dataset(args):
#     """
#     得到Jamming sequence 数据集
#     传参设置:
#     args: ArgumentParser（参数解释器）
#     返回: dataset数据集
#     """
#
#     train_dataset = Jamming_seq_Dataset(args.train_data_dir, args.task_num,
#                                     n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
#     val_dataset = Jamming_seq_Dataset(args.val_data_dir, args.val_task_num,
#                                   n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
#
#     return train_dataset, val_dataset
...
def get_dataset_fusion(args):
    ...
    train_dataset = Jamming_fusion_Dataset(args.train_data_img_dir, args.train_data_seq_dir, args.task_num,
                                    n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
    val_dataset = Jamming_fusion_Dataset(args.val_data_img_dir, args.val_data_seq_dir, args.val_task_num,
                                  n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
...
def get_val_img_dataset(snr, args):
    ...
    val_dataset = Jamming_img_Dataset(val_data_dir, args.val_task_num,
                                  n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
...
def get_val_seq_dataset(snr, args):
    ...
    """
    val_data_dir = args.val_data_dirs[snr]  # 假设 args.val_data_dirs 是一个字典，映射信噪比到数据路径
    val_dataset = Jamming_seq_Dataset(val_data_dir, args.val_task_num,
                                  n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
...
def get_val_fusion_dataset(snr, args):
    """
    返回: val_img_dataset与val_seq_dataset数据集进行测试test用
    """
    ...
    val_dataset = Jamming_fusion_Dataset(val_data_img_dir, val_data_seq_dir, args.val_task_num,
                                  n_way=args.n_way, k_shot=args.k_shot, q_query=args.q_query)
